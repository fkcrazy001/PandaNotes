...
class SegTree:
    ...
    def push_down(self, node,l,r):
        if self.lazy[node] != 0:
            mid = (l+r)>>1
            self.arr[node*2] += self.lazy[node] * (mid-l+1)
            self.arr[node*2+1] += self.lazy[node] * (r-mid)
            self.lazy[node*2+1] += self.lazy[node]
            self.lazy[node*2] += self.lazy[node]
            self.lazy[node] = 0
    def inc(self,node,l,r, ql,qr,v):
        # print("self.inc", node,l,r,ql,qr,v)
        if ql<=l and qr>=r:
            self.arr[node] += (r-l+1)*v
            self.lazy[node] += v
            return
        mid = (l+r)>>1
        self.push_down(node,l,r)
        if mid >= ql:
            self.inc(node*2, l,mid, ql,qr,v)
        if mid+1<=qr:
            self.inc(node*2+1, mid+1,r,ql,qr,v)
    ...
    def query(self,node,l,r, ql, qr):
        ...
        mid = (l+r)>>1
        if mid >= ql:
            s+= self.query(node*2, l, mid, ql,qr)
        if mid+1<=qr:
            s+=self.query(node*2+1,mid+1,r,ql,qr)
        return s
```